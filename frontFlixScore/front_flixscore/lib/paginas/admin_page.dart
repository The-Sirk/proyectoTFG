...
class _AdminUsuariosPageState extends State<AdminUsuariosPage> {
  ...
  Future<void> _toggleDisable(String uid, bool disabled) async {
    try {
      ...
      mostrarSnackBarExito(context, disabled ? 'Usuario suspendido' : 'Usuario habilitado');
    } catch (e) {
    ...
    return Scaffold(
      ...
      body: _cargando && _usuarios.isEmpty
          ...
          : RefreshIndicator(
                    ...
                    return Center(
                      child: Padding(
                        ...
                        child: ElevatedButton(
                          onPressed: _cargandoMas ? null : () => _cargarUsuarios(siguiente: true),
                          child: _cargandoMas
            ...
            return Center(
              child: SizedBox(
                width: MediaQuery.of(context).size.width <= 600 ? double.infinity : MediaQuery.of(context).size.width * 0.5,
                child: Card(
                  ...
                  child: ExpansionTile(
                    ...
                    title: Text(email, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
                    subtitle: Column(
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            Text(rol.toUpperCase(), style: TextStyle(color: roleColor, fontWeight: FontWeight.bold)),
                            const SizedBox(width: 8),
                            Text(disabled ? 'SUSPENDIDO' : 'ACTIVO', style: TextStyle(color: disabled ? Colors.red : Colors.green, fontSize: 12)),
                          ],
                        ),
                      ],
                    ),
                    children: [
                      Padding(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Verificado: ${u['emailVerified'] == true ? 'Sí' : 'No'}', style: const TextStyle(color: Colors.white70)),
                            if (u['creationTime'] != null)
                              Text('Creado: ${DateFormat('dd/MM/yyyy HH:mm').format(DateTime.parse(u['creationTime']).toLocal())}', style: const TextStyle(color: Colors.white70)),
                            if (u['lastSignInTime'] != null)
                              Text('Último acceso: ${DateFormat('dd/MM/yyyy HH:mm').format(DateTime.parse(u['lastSignInTime']).toLocal())}', style: const TextStyle(color: Colors.white70)),
                            const SizedBox(height: 12),
                            const Text('Rol:', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                            DropdownButton<String>(
                              dropdownColor: Colors.grey[850],
                              value: ['user', 'admin'].contains(rol) ? rol : 'user',
                              items: ['user', 'admin'].map((r) => DropdownMenuItem(value: r, child: Text(r.toUpperCase(), style: const TextStyle(color: Colors.white)))).toList(),
                              onChanged: (val) {
                              ...
                              children: [
                                SizedBox(
                                  ...
                                  child: ElevatedButton.icon(
                                    onPressed: disabled ? null : () => _resetPassword(uid),
                                ...
                                SizedBox(
                                  ...
                                  child: ElevatedButton.icon(
                                    ...
                                    icon: Icon(disabled ? Icons.check_circle : Icons.block),
                                    label: Text(disabled ? 'Habilitar' : 'Deshabilitar'),
                                    style: ElevatedButton.styleFrom(
                                ...
                                SizedBox(
                                  width: 300,
                                  child: ElevatedButton.icon(
                                    onPressed: disabled ? null : () => _eliminarUsuario(uid),
